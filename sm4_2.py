from typing import List

from method.gmalg.func import ROL32

_S_BOX = bytes([
    
])

_CK = [
    0x00070E15, 0x1C232A31, 0x383F464D, 0x545B6269, 0x70777E85, 0x8C939AA1, 0xA8AFB6BD, 0xC4CBD2D9,
    0xE0E7EEF5, 0xFC030A11, 0x181F262D, 0x343B4249, 0x50575E65, 0x6C737A81, 0x888F969D, 0xA4ABB2B9,
    0xC0C7CED5, 0xDCE3EAF1, 0xF8FF060D, 0x141B2229, 0x30373E45, 0x4C535A61, 0x686F767D, 0x848B9299,
    0xA0A7AEB5, 0xBCC3CAD1, 0xD8DFE6ED, 0xF4FB0209, 0x10171E25, 0x2C333A41, 0x484F565D, 0x646B7279
]


def _BS(X):
    return ((_S_BOX[(X >> 24) & 0xff] << 24) |
            (_S_BOX[(X >> 16) & 0xff] << 16) |
            (_S_BOX[(X >> 8) & 0xff] << 8) |
            (_S_BOX[X & 0xff]))

# 写一个a+b


def _T0(X):
    X = _BS(X)
    return X ^ ROL32(X, 2) ^ ROL32(X, 10) ^ ROL32(X, 18) ^ ROL32(X, 24)


def _T1(X):
    X = _BS(X)
    return X ^ ROL32(X, 13) ^ ROL32(X, 23)


def _key_expand(key: bytes, rkey: List[int]):
    """Key expansion."""

    K0 = int.from_bytes(key[0:4], "big") ^ 0xa3b1bac6
    K1 = int.from_bytes(key[4:8], "big") ^ 0x56aa3350
    K2 = int.from_bytes(key[8:12], "big") ^ 0x677d9197
    K3 = int.from_bytes(key[12:16], "big") ^ 0xb27022dc

    for i in range(0, 32, 4):
        K0 = K0 ^ _T1(K1 ^ K2 ^ K3 ^ _CK[i])
        rkey[i] = K0
        K1 = K1 ^ _T1(K2 ^ K3 ^ K0 ^ _CK[i + 1])
        rkey[i + 1] = K1
        K2 = K2 ^ _T1(K3 ^ K0 ^ K1 ^ _CK[i + 2])
        rkey[i + 2] = K2
        K3 = K3 ^ _T1(K0 ^ K1 ^ K2 ^ _CK[i + 3])
        rkey[i + 3] = K3


class SM4():
    """SM4 Algorithm."""

    @classmethod
    def key_length(self) -> int:
        """Get key length in bytes."""

        return 16

    @classmethod
    def block_length(self) -> int:
        """Get block length in bytes."""

        return 16

    def __init__(self, key: bytes, iv: bytes) -> None:
        """SM4 Algorithm.

        Args:
            key: 16 bytes key.

        Raises:
            IncorrectLengthError: Incorrect key length.
        """

        self._key: bytes = key
        self._iv: bytes = iv
        self._rkey: List[int] = [0] * 32
        _key_expand(self._key, self._rkey)

        self._block_buffer = bytearray()

    def encrypt(self, block: bytes) -> bytes:
        """Encrypt.

        Args:
            block: Plain block to encrypt, must be 16 bytes.

        Returns:
            bytes: 16 bytes cipher block.

        Raises:
            IncorrectLengthError: Incorrect block length.
        """

        RK = self._rkey

        X0 = int.from_bytes(block[0:4], "big")
        X1 = int.from_bytes(block[4:8], "big")
        X2 = int.from_bytes(block[8:12], "big")
        X3 = int.from_bytes(block[12:16], "big")

        for i in range(0, 32, 4):
            X0 = X0 ^ _T0(X1 ^ X2 ^ X3 ^ RK[i])
            X1 = X1 ^ _T0(X2 ^ X3 ^ X0 ^ RK[i + 1])
            X2 = X2 ^ _T0(X3 ^ X0 ^ X1 ^ RK[i + 2])
            X3 = X3 ^ _T0(X0 ^ X1 ^ X2 ^ RK[i + 3])

        BUFFER = self._block_buffer
        BUFFER.clear()
        BUFFER.extend(X3.to_bytes(4, "big"))
        BUFFER.extend(X2.to_bytes(4, "big"))
        BUFFER.extend(X1.to_bytes(4, "big"))
        BUFFER.extend(X0.to_bytes(4, "big"))
        return bytes(BUFFER)

    def decrypt(self, block: bytes) -> bytes:
        """Decrypt.

        Args:
            block: cipher block to decrypt, must be 16 bytes.

        Returns:
            bytes: 16 bytes plain block.

        Raises:
            IncorrectLengthError: Incorrect block length.
        """

        RK = self._rkey

        X0 = int.from_bytes(block[0:4], "big")
        X1 = int.from_bytes(block[4:8], "big")
        X2 = int.from_bytes(block[8:12], "big")
        X3 = int.from_bytes(block[12:16], "big")

        for i in range(0, 32, 4):
            X0 = X0 ^ _T0(X1 ^ X2 ^ X3 ^ RK[31 - i])
            X1 = X1 ^ _T0(X2 ^ X3 ^ X0 ^ RK[30 - i])
            X2 = X2 ^ _T0(X3 ^ X0 ^ X1 ^ RK[29 - i])
            X3 = X3 ^ _T0(X0 ^ X1 ^ X2 ^ RK[28 - i])

        BUFFER = self._block_buffer
        BUFFER.clear()
        BUFFER.extend(X3.to_bytes(4, "big"))
        BUFFER.extend(X2.to_bytes(4, "big"))
        BUFFER.extend(X1.to_bytes(4, "big"))
        BUFFER.extend(X0.to_bytes(4, "big"))
        return bytes(BUFFER)

    def pad(self, data: bytes) -> bytes:
        """Apply PKCS7 padding to the input data."""
        padding_length = 16 - (len(data) % 16)
        return data + bytes([padding_length] * padding_length)

    def unpad(self, data: bytes) -> bytes:
        """Remove PKCS7 padding from the input data."""
        padding_length = data[-1]
        if padding_length > 16:
            raise ValueError("Invalid padding")
        return data[:-padding_length]

    def encrypt_cbc(self, plaintext: bytes) -> bytes:
        """Encrypt plaintext using SM4 in CBC mode with PKCS7 padding.

        Args:
            plaintext (bytes): The plaintext to be encrypted.

        Returns:
            bytes: Encrypted ciphertext.
        """
        # Apply PKCS7 padding
        padded_plaintext = self.pad(plaintext)
        # print(f"PKCS7填充后的明文{padded_plaintext}")

        blocks = [padded_plaintext[i:i + 16] for i in range(0, len(padded_plaintext), 16)]
        cipher_blocks = []
        previous_block = self._iv  # Initialize with IV

        for block in blocks:
            # XOR current block with previous ciphertext (or IV for first block)
            xored_block = bytes(a ^ b for a, b in zip(block, previous_block))
            cipher_block = self.encrypt(xored_block)  # Encrypt the xored result
            cipher_blocks.append(cipher_block)
            previous_block = cipher_block  # Set current ciphertext as previous for next round

        return b''.join(cipher_blocks)

    def decrypt_cbc(self, ciphertext: bytes) -> bytes:
        """Decrypt ciphertext using SM4 in CBC mode with PKCS7 unpadding.

        Args:
            ciphertext (bytes): The ciphertext to be decrypted. Must be a multiple of 16 bytes.

        Returns:
            bytes: Decrypted plaintext.
        """
        if len(ciphertext) % 16 != 0:
            raise ValueError("Ciphertext length must be a multiple of 16 bytes")

        blocks = [ciphertext[i:i + 16] for i in range(0, len(ciphertext), 16)]
        plain_blocks = []
        previous_block = self._iv  # Initialize with IV

        for block in blocks:
            decrypted_block = self.decrypt(block)  # Decrypt the current block
            # XOR decrypted block with previous ciphertext (or IV for first block)
            plain_block = bytes(a ^ b for a, b in zip(decrypted_block, previous_block))
            plain_blocks.append(plain_block)
            previous_block = block  # Use current ciphertext block for next round

        # Join the decrypted blocks and remove padding
        decrypted_data = b''.join(plain_blocks)
        # print(f"解密后的数据{decrypted_data}")
        return self.unpad(decrypted_data)
        # return decrypted_data
